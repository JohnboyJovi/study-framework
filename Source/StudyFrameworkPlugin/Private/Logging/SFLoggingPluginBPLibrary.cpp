...
USFLoggingPluginBPLibrary::USFLoggingPluginBPLibrary(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer) {
}

void USFLoggingPluginBPLibrary::LogToOneFile(int32 Timer) {
	//HIER ÄNDERN!!
	if (!(FModuleManager::Get().IsModuleLoaded("LoggingPlugin"))) {
		return;
	}
	FLoggingPluginModule& LoggingPluginModule = FModuleManager::GetModuleChecked<FLoggingPluginModule>("LoggingPlugin");
	USFLogObject* Instance = LoggingPluginModule.GetInstance();
	if (Instance->LogThis == false) {
		return;
	}
	// if(Should logging start yet?)
	if ((FDateTime::Now() - Instance->StaticDateTime).GetTotalMilliseconds() > Timer || Timer == 0) {
		...
		FString Row = "";
		for (auto& ActorLoggingInfo : LoggingPluginModule.GetInstance()->LoggingInfo) {
			if (FirstActor == true) {
				FirstActor = false;
...
void USFLoggingPluginBPLibrary::LogToFile() {
	if (!(FModuleManager::Get().IsModuleLoaded("LoggingPlugin"))) {
		return;
	}
	FLoggingPluginModule& LoggingPluginModule = FModuleManager::GetModuleChecked<FLoggingPluginModule>("LoggingPlugin");
...
void USFLoggingPluginBPLibrary::AddActorToLog(AActor* Actor, int32 LogTimer) {
	if (FModuleManager::Get().IsModuleLoaded("LoggingPlugin")) {
		FLoggingPluginModule& LoggingPluginModule = FModuleManager::GetModuleChecked<FLoggingPluginModule>("LoggingPlugin");
		if (LoggingPluginModule.GetInstance()->LogThis == false) {
			return;
		}
		LoggingPluginModule.GetInstance()->AddActor(Actor,(++LogTimer));
	}
	else {
...
void USFLoggingPluginBPLibrary::AddActorToLogWithName(AActor* Actor, int32 LogTimer, FString LogName) {
	if (FModuleManager::Get().IsModuleLoaded("LoggingPlugin")) {
		FLoggingPluginModule& LoggingPluginModule = FModuleManager::GetModuleChecked<FLoggingPluginModule>("LoggingPlugin");
		if (LoggingPluginModule.GetInstance()->LogThis == false) {
			return;
		}
		LoggingPluginModule.GetInstance()->AddActorWithName(Actor, (++LogTimer), LogName);
	}
	else {
		UE_LOG(LogTemp, Warning, TEXT("Es ist ein Fehler aufgetreten - Der Actor konnte nicht hinzugefuegt werden!"));
	}
...
void ULoggingPluginBPLibrary::AddActorsToLog(TArray<AActor*> ActorArray, int32 LogTimer) {
	if (FModuleManager::Get().IsModuleLoaded("LoggingPlugin")) {
		FLoggingPluginModule& LoggingPluginModule = FModuleManager::GetModuleChecked<FLoggingPluginModule>("LoggingPlugin");
		if (LoggingPluginModule.GetInstance()->LogThis == false) {
			return;
		}
		LoggingPluginModule.GetInstance()->AddActors(ActorArray, (++LogTimer));
	}
	else {
		UE_LOG(LogTemp, Warning, TEXT("Es ist ein Fehler aufgetreten - Der Actor konnte nicht hinzugefuegt werden!"));
	}
...
void USFLoggingPluginBPLibrary::Initialize() {
	if (FModuleManager::Get().IsModuleLoaded("LoggingPlugin")) {
		FLoggingPluginModule& LoggingPluginModule = FModuleManager::GetModuleChecked<FLoggingPluginModule>("LoggingPlugin");
		LoggingPluginModule.GetInstance()->Initialize();
	}
}

void USFLoggingPluginBPLibrary::LogCustomEvent(FString EventName) {
	if (!(FModuleManager::Get().IsModuleLoaded("LoggingPlugin"))) {
		return;
	}
	FLoggingPluginModule& LoggingPluginModule = FModuleManager::GetModuleChecked<FLoggingPluginModule>("LoggingPlugin");
...
void USFLoggingPluginBPLibrary::LogCustomEventWithActor(FString EventName, AActor* Actor) {
	if (!(FModuleManager::Get().IsModuleLoaded("LoggingPlugin"))) {
		return;
	}
	FLoggingPluginModule& LoggingPluginModule = FModuleManager::GetModuleChecked<FLoggingPluginModule>("LoggingPlugin");
...
void USFLoggingPluginBPLibrary::LogCustomEventWithActorAndName(FString EventName, AActor* Actor, FString ActorName) {
	if (!(FModuleManager::Get().IsModuleLoaded("LoggingPlugin"))) {
		return;
	}
	FLoggingPluginModule& LoggingPluginModule = FModuleManager::GetModuleChecked<FLoggingPluginModule>("LoggingPlugin");
